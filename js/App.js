...
window.onload = async () => {
  ...
  if (window.ethereum) {
    ...
    console.log("dd: ",window.userAddress);

    if (window.userAddress.length ? window.userAddress.length > 10 : false) {
...
function generateQRCode() {
  ...
  let url = `${window.location.host}/AdminUpload/verify.html?hash=${window.hashedfile}`
  qrcode.makeCode(url)
